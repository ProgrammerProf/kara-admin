"use client";
import { api, date, file_info, matching, host, alert_msg, print } from '@/public/script/public';
...
export default function Mail () {
    ...
    const searchMails = ( current, reset=true ) => {
        ...
        _data_ = _data_.filter((item) => 
            ...
            matching(item.date, search) ||
            matching(date_time(item.date), search) ||
            matching(item.star ? 'star' : '', search) ||
            matching(item.important ? 'important' : '', search) ||
            matching(item.sender?.name, search) ||
            matching(item.sender?.email, search) ||
            matching(item.receiver?.name, search) ||
            matching(item.receiver?.email, search) ||
            matching(item.sender?.online ? 'online' : 'offline', search) ||
            matching(item.receiver?.online ? 'online' : 'offline', search)
        )
    ...
    const actions = async( action, id ) => {
        ...
        if ( action === 'readen' ) {
            ...
            if ( !ids.length ) return;
            let _data_ = type === 'send' ? send : inbox;
            _data_ = _data_.map(_ => { ids.includes(_.id) ? _.active = true : ''; return _ });
            type === 'send' ? setSend(_data_) : setInbox(_data_);
        ...
        if ( action === 'unread' ) {
            ...
            if ( !ids.length ) return;
            let _data_ = type === 'send' ? send : inbox;
            _data_ = _data_.map(_ => { ids.includes(_.id) ? _.active = false : ''; return _ });
            type === 'send' ? setSend(_data_) : setInbox(_data_);
                            ...
                            <div className=" pt-2 flex flex-wrap default">
                                ...
                                <div className="px-2 flex-1">
                                    ...
                                    <div className="flex items-center -mt-[4px]">
                                        ...
                                        <div className="text-white-dark whitespace-nowrap">{date_time(selected.date)}</div>

                                    </div>

                                    <div className="text-white-dark flex items-center mt-[2px]">

                                        <div className="ltr:mr-1 rtl:ml-1">{selected.sender?.email || ''}</div>
                                                ...
                                                <tr key={index} className="cursor-pointer" onClick={() => actions('open', item.id)}>
                                                    <td>
                                                        <div className="flex items-center whitespace-nowrap">
                                                            ...
                                                            <div className={`whitespace-nowrap font-semibold dark:text-gray-300 flex items-center mx-3 ${!mail.isUnread ? 'text-gray-500 dark:text-gray-500' : ''}`}>
                                                               
                                                                <div className="rounded-full w-max ltr:mr-3 rtl:ml-3">
                                                                  
                                                                    <img 
                                                                        src={`${host}/U${item.sender?.id}`} 
                                                                        onError={(e) => e.target.src = "/media/public/user_icon.png"}
                                                                        onLoad={(e) => e.target.src.includes('_icon') ? e.target.classList.add('empty') : e.target.classList.remove('empty')}
                                                    ...
                                                    <td>
                                                        <div className="min-w-[300px] overflow-hidden font-medium text-white-dark line-clamp-1">
                                                            ...
                                                            <span className={`${item.active ? '' : 'font-semibold text-gray-800 dark:text-gray-300'}`}>
