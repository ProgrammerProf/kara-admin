...
export default function Form_Owner ({ id }) {
    ...
    const get_item = async() => {
        ...
        setLoader(false);
        document.title = `Edit Owner | ${response.data.name || ''}`;

    }
    const save_item = async() => {
        
        if ( !data.name ) return alert_msg('Error, owner name required !', 'error');
        if ( !data.email ) return alert_msg('Error, owner e-mail required !', 'error');
        ...
        if ( response.status === true ) {
            ...
            else alert_msg(`New owner added successfully`);
            return router.replace('/owners')
        }
        else if ( response.status === 'exists' ) alert_msg('This owner e-mail is already exists !', 'error');
    ...
    useEffect(() => {
        ...
        document.title = id ? 'Edit Owner' : 'Add Owner';
                            ...
                            <div className="mt-4 px-4">
                                ...
                                <div className="flex justify-between lg:flex-row flex-col">
                                    ...
                                    <div className="lg:w-1/2 w-full ltr:lg:mr-6 rtl:lg:ml-6 mb-4 div-2">
                                        ...
                                        <div className="flex items-center mt-4">
                                            <label htmlFor="products" className="ltr:mr-2 rtl:ml-2 w-1/4 mb-0">Properties</label>
                                            <input id="products" type="number" min='0' value={data.products || 0} readOnly className="form-input flex-1 default" autoComplete="off"/>
                                        </div>

                                        <div className="flex items-center mt-4">
                                            <label htmlFor="visitors" className="ltr:mr-2 rtl:ml-2 w-1/4 mb-0">Visitors</label>
                                            <input id="visitors" type="number" min='0' value={data.visitors || 0} readOnly className="form-input flex-1 default" autoComplete="off"/>
                                        </div>
                                    ...
                                    <div className="lg:w-1/2 w-full div-3">
                                        ...
                                        <div className="flex items-center mt-4">
                                            <label htmlFor="balance" className="ltr:mr-2 rtl:ml-2 w-1/3 mb-0 ltr:pl-8 rtl:pr-8">Balance</label>
                                            <input id="balance" type="number" min='0' value={data.balance || 0} readOnly className="form-input flex-1 default" autoComplete="off"/>
                                        </div>

                                        <div className="flex items-center mt-4">
                                            <label htmlFor="orders" className="ltr:mr-2 rtl:ml-2 w-1/3 mb-0 ltr:pl-8 rtl:pr-8">Bookings</label>
